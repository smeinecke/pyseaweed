...
from pyseaweed._common import prepare_stream, range_headers
...
class Filer:
    ...
    def set_tags(self, remote_path: str, tags: dict[str, str]) -> bool:
        ...
        """
        headers = {f"{_TAG_PREFIX}{name}": str(value) for name, value in tags.items()}
    ...
    def get_tags(self, remote_path: str) -> dict[str, str] | None:
        ...
        for key, value in extended.items():
            if key.startswith(_TAG_PREFIX):
                try:
                    tags[key[len(_TAG_PREFIX) :]] = base64.b64decode(value).decode()
                except ValueError:
                    tags[key[len(_TAG_PREFIX) :]] = value
        return tags
    ...
    def delete_tags(self, remote_path: str, names: Iterable[str] | None = None) -> bool:
        ...
        """
        tagging = "" if names is None else ",".join(_canonical_tag_name(name) for name in names)
